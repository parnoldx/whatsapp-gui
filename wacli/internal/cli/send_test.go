...
func TestPersistOutboundTextStoresReplyContext(t *testing.T) {
	db := openSendTestDB(t)
	chat := types.NewJID("15559999999", types.GroupServer)
	now := time.Date(2026, 7, 27, 12, 0, 0, 0, time.UTC)

	if err := db.UpsertChat(chat.String(), "group", "Crew", now); err != nil {
		t.Fatalf("UpsertChat: %v", err)
	}
	if err := persistOutboundTextWith(context.Background(), db, outboundTextResolverStub{}, chat, "reply-out", "a reply", now, "quoted-id", "15559999999@s.whatsapp.net"); err != nil {
		t.Fatalf("persistOutboundTextWith: %v", err)
	}
	stored, err := db.GetMessage(chat.String(), "reply-out")
	if err != nil {
		t.Fatalf("GetMessage: %v", err)
	}
	if stored.QuotedMsgID != "quoted-id" || stored.QuotedSenderJID != "15559999999@s.whatsapp.net" {
		t.Fatalf("reply context lost: quoted=%q sender=%q", stored.QuotedMsgID, stored.QuotedSenderJID)
	}
}
