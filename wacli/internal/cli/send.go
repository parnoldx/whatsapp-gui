...
func newSendTextCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			now := time.Now().UTC()
			chat := toJID
			storeErr := persistOutboundText(ctx, a, chat, string(msgID), message, now, replyTo, replyToSender)
...
func persistOutboundText(ctx context.Context, a *app.App, chat types.JID, msgID, text string, now time.Time, replyTo, replyToSender string) error {
	return persistOutboundTextWith(ctx, a.DB(), a.WA(), chat, msgID, text, now, replyTo, replyToSender)
}
...
func persistOutboundTextWith(ctx context.Context, db *store.DB, resolver outboundTextResolver, chat types.JID, msgID, text string, now time.Time, replyTo, replyToSender string) error {
	...
	if err := db.UpsertMessage(store.UpsertMessageParams{
		...
		FromMe:          true,
		Text:            text,
		QuotedMsgID:     strings.TrimSpace(replyTo),
		QuotedSenderJID: strings.TrimSpace(replyToSender),
	}); err != nil {
