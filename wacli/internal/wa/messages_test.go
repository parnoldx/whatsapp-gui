...
func TestParseLiveMessageRevokeByOtherIsNotFromMe(t *testing.T) {
	chat := types.NewJID("491737074325-1368379510", types.GroupServer)
	sender := types.NewJID("491621901830", types.DefaultUserServer)
	ev := &events.Message{
		Info: types.MessageInfo{
		...
		Message: &waProto.Message{
			ProtocolMessage: &waProto.ProtocolMessage{
				...
				Key: &waProto.MessageKey{
					ID:          proto.String("original"),
					FromMe:      proto.Bool(true), // relative to the revoking participant
					RemoteJID:   proto.String(chat.String()),
	...
	pm := ParseLiveMessage(ev)
	if !pm.Revoked || pm.FromMe {
		t.Fatalf("revoke by another participant must not be from me: %+v", pm)
	}
}
